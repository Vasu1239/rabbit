...
def voice_component():
    """
    Voice + text input that returns typed/spoken query to Python.
    Uses Streamlit.setComponentValue() — the only reliable iframe→Python bridge.
    """
    html_code = """
<!DOCTYPE html>
<html>
...
<style>
  * { margin:0; padding:0; box-sizing:border-box; }
  body {
    ...
    padding: 4px 2px; font-family: sans-serif; overflow: hidden;
  }
  #inp {
  ...
  #inp:focus { border-color:rgba(139,92,246,.8); box-shadow:0 0 0 3px rgba(139,92,246,.15); }
  ...
  #sendBtn {
    height:42px; padding:0 18px; width:auto;
    background: linear-gradient(135deg,#8B5CF6,#06B6D4);
    color:#fff; font-weight:700; font-size:13px; letter-spacing:.04em;
    box-shadow:0 4px 14px rgba(139,92,246,.35); border-radius:11px;
  }
  #sendBtn:hover { transform:translateY(-1px); box-shadow:0 6px 22px rgba(139,92,246,.55); }
  #status { font-size:11px; color:#F472B6; font-weight:600;
            letter-spacing:.07em; text-transform:uppercase; white-space:nowrap; min-width:66px; }
</style>
</head>
<body>
<input id="inp" type="text" placeholder="🎤 Speak or type your question and press Send…" />
<span id="status"></span>
<button class="btn" id="micBtn" title="Click to speak">🎤</button>
...
document.getElementById('micBtn').addEventListener('click', function() {
  ...
  if (!SR) { document.getElementById('status').textContent='No mic'; return; }
  recog = new SR();
  ...
  recog.onresult = function(e) {
    ...
    if(e.results[e.results.length-1].isFinal) {
      ...
      setTimeout(function(){ sendQuery(t); },400);
    }
  };
  recog.onerror=function(e){ document.getElementById('status').textContent='Err:'+e.error; };
...
document.getElementById('sendBtn').addEventListener('click', function(){
  sendQuery(document.getElementById('inp').value);
});
document.getElementById('inp').addEventListener('keydown', function(e){
  if(e.key==='Enter') sendQuery(this.value);
});
...
"""
    return components.html(html_code, height=54, scrolling=False)
...
# ════════════════════════════════════════════════
    ...
    # ── VOICE + TEXT INPUT ──────────────────────
    st.markdown('<p style="font-size:.57rem;color:#1E293B;letter-spacing:.14em;text-transform:uppercase;margin-bottom:.4rem;">🎤 Speak or type — press Send or Enter</p>', unsafe_allow_html=True)

    # Voice component returns the query string when user speaks/sends
    voice_result = voice_component()

    # Determine query: sidebar suggestion > voice/text component result
    query_to_run = ""
    if st.session_state.pending_query:
        query_to_run = st.session_state.pending_query
        st.session_state.pending_query = ""
    elif voice_result and isinstance(voice_result, str) and voice_result.strip():
        if voice_result != st.session_state.get("last_voice", ""):
            query_to_run = voice_result.strip()
            st.session_state.last_voice = voice_result
