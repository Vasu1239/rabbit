...
# SESSION STATE
# ─────────────────────────────────────────────
for k, v in [("chat_history", []), ("df", None), ("api_key", "")]:
...
# ─────────────────────────────────────────────
    ...
    # Inline API key input if not yet set
    if not st.session_state.api_key:
        ...
        <div style="background:rgba(139,92,246,0.07);border:1px solid rgba(139,92,246,0.25);
            ...
            <div style="font-size:0.7rem;font-weight:600;color:#A78BFA;letter-spacing:0.1em;
                        text-transform:uppercase;margin-bottom:0.5rem;">🔑 Enter your Anthropic API Key to enable AI</div>
            <div style="font-size:0.78rem;color:#475569;">
                Get your key at <b style="color:#8B5CF6;">console.anthropic.com</b> → API Keys
            </div>
        </div>
        """, unsafe_allow_html=True)
        key_input = st.text_input("API Key", placeholder="sk-ant-...", type="password",
                                   label_visibility="collapsed", key="key_field")
        if st.button("Connect AI →", use_container_width=False):
